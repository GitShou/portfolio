    ...
    detail: {
      ...
      sections: [
        ...
        },
        {
          ...
          good: [
            {
              title: "ページ遷移ビーコンによる行動分析の強化",
              description:
                "Next.js App RouterのSPA遷移では通常のページリクエストログだけでは追跡が難しいため、遷移時にビーコンを送信する仕組みを導入。CloudFront経由でAPI Gateway/Lambdaへ連携し、S3に蓄積したイベントをAthenaで分析できるようにした。"
            },
            {
              title: "マルチリージョン連携",
              description:
            ...
            {
              title: "エラー時のフェイルオーバー設定",
              description:
                "今回はステージング環境を用意していないため、アップデートで本番環境でエラーが起こることもあるが、Cloudfrontでフェイルオーバーの設定を行い、閲覧者にメンテナンス中であることを通知できるようにした。"
            },
            {
            ...
            {
              title: "CloudFormationのスタックの構築範囲を最適化",
              description:
                "上記フェイルオーバーを実現するにあたり、もともと1つのスタックで作成していたフロントエンドを分割し、エッジ、コンテンツ、エラーの3スタックに分離した。"
            },
            {
              title: "テンプレート駆動開発の徹底",
              description:
        ...
        },
        {
          ...
          details: [
            {
              heading: "開発期間と工数",
              body: "平日1日平均3時間で開発を進め、着手開始からWebサイト公開までの時間はトータルで100時間程度で本システムをフルスタックで完成させた。\n空白の時間を見つけ、今後もWebサイトとしての最適化を実施中。"
            },
            {
              heading: "成果・学び",
    ...
    detail: {
      ...
      sections: [
        ...
        },
        {
          ...
          body:
            "本プロジェクトの内容を『AWS Lambda を使ったサーバレスのアプリケーション開発』として資料化し、社外フォーラムで発表。複雑な技術課題を体系的に整理し、他者に分かりやすく伝えるスキルも証明。"
        }
      ]
    }
  },
